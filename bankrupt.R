...
#' setwd("~/Documents/FinancialDatamining/homework/week5/assignment_05/R_version")
source('sigmoid.R')
source('computeCost.R')
source('grad.R')
#######################################################
#' loading data
data <- read.csv('/Users/lihao/Documents/FinancialDatamining/homework/week5/assignment_05/R_version/bankruptcy.csv', 
                 header = TRUE)
...
theta <- optim(par = init, fn = computeCost, gr = grad, X = Xtrain, y = ytrain,
              lambda = lambda, method = 'BFGS', control = list(maxit = 50))$par
#' prediction and accuracy
pred <- sigmoid(Xtest %*% theta) >= 0.5
length(ytest[as.numeric(pred) == ytest]) / length(ytest)
